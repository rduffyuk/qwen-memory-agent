...
# Title
ax.text(75, 96, "Qwen MemoryAgent — Architecture",
        ha="center", va="center", fontsize=20, fontweight="bold", color=INK)
ax.text(75, 91.5, "Track 1 · MemoryAgent  ·  agentic memory on Qwen Cloud, deployed on Alibaba Cloud ECS",
        ha="center", va="center", fontsize=11, color="#5a6b82")

# Client (left)
client = box(5, 46, 22, 12, CLIENT, CLIENT_EDGE,
             "Client", "MCP client · demo UI · curl", tsize=14, ssize=9)

# ECS container (center)
ecs_x, ecs_y, ecs_w, ecs_h = 38, 12, 62, 72
ax.add_patch(FancyBboxPatch((ecs_x, ecs_y), ecs_w, ecs_h,
             boxstyle="round,pad=0.02,rounding_size=3",
             facecolor=ECS, edgecolor=ECS_EDGE, linewidth=2.5, linestyle="--"))
ax.text(ecs_x + ecs_w / 2, ecs_y + ecs_h - 3.5,
        "Alibaba Cloud ECS  (Singapore · 2 vCPU / 3.4 GiB · Ubuntu)",
        ha="center", va="center", fontsize=11.5, fontweight="bold", color=ECS_EDGE)

cx0 = ecs_x + 5
cw = ecs_w - 10
fastapi = box(cx0, 66, cw, 9, COMP, COMP_EDGE,
              "FastAPI backend", "/chat · /health · /usage · /memory/* · /dream · /dream/apply",
              tsize=13, ssize=8.5)
agent = box(cx0, 52, cw * 0.5 - 2, 9, COMP, COMP_EDGE,
            "MemoryAgent loop", "Qwen function-calling", tsize=12, ssize=9)
mcp = box(cx0 + cw * 0.5 + 2, 52, cw * 0.5 - 2, 9, COMP, COMP_EDGE,
          "FastMCP server", "remember / recall / forget / stats\nexport / import / dream / dream_apply", tsize=12, ssize=7.2)
engine = box(cx0, 33, cw, 13, ENGINE, ENGINE_EDGE,
             "Memory Engine",
             ...
             tsize=13, ssize=7.8)
qdrant = box(cx0, 17, cw * 0.5 - 2, 9, QD, QD_EDGE,
             "Qdrant", "vector store (embedded)", tsize=12, ssize=9)
persist = box(cx0 + cw * 0.5 + 2, 17, cw * 0.5 - 2, 9, "#fdf2f8", "#d6337a",
              "Disk snapshot", "memory.json · survives restart", tsize=12, ssize=8.2)

# Qwen Cloud (right)
qwen = box(112, 34, 34, 30, CLOUD, CLOUD_EDGE, "", "", radius=3, lw=2.5)
ax.text(129, 58, "Qwen Cloud", ha="center", va="center", fontsize=15, fontweight="bold", color=CLOUD_EDGE)
ax.text(129, 54, "DashScope-intl", ha="center", va="center", fontsize=10.5, color="#42505f")
ax.text(129, 54, "", ha="center")
ax.text(129, 47.5, "OpenAI-compatible endpoint", ha="center", va="center", fontsize=9, style="italic", color="#5a6b82")
ax.text(129, 43, "qwen-plus", ha="center", va="center", fontsize=10.5, fontweight="bold", color=INK)
ax.text(129, 40, "reasoning + tool-calling", ha="center", va="center", fontsize=8.5, color="#42505f")
ax.text(129, 36.5, "text-embedding-v3", ha="center", va="center", fontsize=10.5, fontweight="bold", color=INK)
...
arrow((cx0 + cw * 0.74, 33), (cx0 + cw * 0.74, 26), "", color="#d6337a")  # Engine -> snapshot (save)
arrow((cx0 + cw * 0.80, 26), (cx0 + cw * 0.80, 33), "", color="#d6337a")  # snapshot -> Engine (load)
ax.text(cx0 + cw * 0.90, 29.5, "save on write /\nload on start", ha="left", va="center",
        fontsize=8, style="italic", color="#d6337a")
...
# footer
ax.text(75, 6.5, "github.com/rduffyuk/qwen-memory-agent  ·  MIT  ·  fully-offline test suite (zero API spend)",
        ha="center", va="center", fontsize=9.5, color="#7a8699")
